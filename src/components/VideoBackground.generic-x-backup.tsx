  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      float sdBetterX(vec3 p) {
        ...
        float barA = sdRoundBox(armA, vec3(1.08, 0.16, 0.22), 0.085);
        float barB = sdRoundBox(armB, vec3(1.08, 0.16, 0.22), 0.085);

        return smin(barA, barB, 0.22);
      }
      ...
      void main() {
        ...
        col += vec3(0.17, 0.13, 0.24) * crossGlow * 0.18;

        if (hit) {
          vec3 n = betterXNormal(p);
          ...
          float iriT = dot(n, viewDir) * 0.5 + uTime * 0.05 + p.y * 0.08 + p.x * 0.04;
          vec3 iri = iridescent(iriT, rim);
          float innerGlow = pow(max(1.0 - abs(p.z) / 0.22, 0.0), 2.0);
          ...
          col += vec3(1.0, 0.82, 0.54) * innerGlow * 0.08;
        } else {
          float halo = exp(-4.5 * abs(length(uv) - 0.32));
          col += vec3(0.16, 0.12, 0.20) * halo * 0.12;
        }
